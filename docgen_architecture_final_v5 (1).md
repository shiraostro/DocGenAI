...
│     reference_id, 
│     job_id,
│     template: { sections, instructions, api_endpoints } 
│   }

Step 2 (FetchMetadata):
├── Input:  { ..., template.api_endpoints }
├── Output: { ..., metadata: { 
│              data: { case_number, case_name, ... },
│              files: [
...
├── Input:  metadata.files[] + job_id
├── Output: { ..., fetched_files: [
...
│            ]}
├── S3 JSON: {
│     source_file, pages_count, extracted_at,
│     text: "...",
│     pages: [{ page: 1, text: "..." }, ...]
│   }

Step 5 (SummarizeContent - MAP):
├── Input:  extracted[] + job_id
├── Output: { ..., summaries: [
│              { 
│                source_file: "f-001.pdf",
...
│            ]}
├── S3 JSON: {
│     source_file, source_s3_key, summarized_at,
│     token_count,
│     summary: "..."
│   }

Step 6 (RedactPII - MAP):
├── Input:  summaries[] + job_id
├── Output: { ..., redacted: [
│              { 
│                source_file: "f-001.pdf",
...
│            ]}
├── S3 JSON: {
│     source_file, source_s3_key, redacted_at,
│     redaction_count,
│     redactions: [{ type, original, replacement }, ...],
│     redacted_text: "..."
│   }

Step 7 (GenerateParagraphs):
├── Input:  redacted[] + template.instructions + job_id
├── Output: { ..., 
...
├── Input:  paragraphs_s3_key + metadata + job_id
├── Output: { 
│     document_s3_key: "jobs/{job_id}/output/document.json",
│     callback_status: "sent"
│   }
├── S3 JSON (מסמך שלם): {
│     job_id: "docgen-abc123",
│     doc_type: "summary_report",
│     reference_id: "case-123",
│     status: "COMPLETED",
...
│     }
│   }
├── Callback: POST to CALLBACK_URL (same JSON)
```
